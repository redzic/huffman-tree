...
pub fn merge_trees(
    ...
    let mut v = vec![Some(
        NonZeroUsize::new(tree[0].unwrap().get() + nnode[0].unwrap().get()).unwrap(),
    )];
...
fn main() -> Result<(), std::io::Error> {
    ...
    // TODO maybe try to store table with a more compact representation,
    // only take 1 bit to specify whether a value is present or not.
    // this will make the table not byte-aligned though.

    // could also experiment with storing the table with a bunch of
